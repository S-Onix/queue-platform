 ...
 * 문제: 구 {@code frontSeq}·{@code total}(ZCARD)은 30만 ZSet 접근이었다.
 *       §36 복귀가 있던 시절엔 만료자가 돌아와 순번이 후퇴하기도 했다.
 * 해결: {@code lastAdmittedSeq} 는 클 때만 올라가 후퇴하지 않고, 한 키 O(1) 이라 MGET 한 번이다.
 * ⚠️ 캐시가 아니라 원본이다 — Redis 유실 시 0으로 돌아가 전원 순번이 폭증한다(복구는 §71·§79).
 *
 * @author sonix
 * @param lastAdmittedSeq 마지막으로 admit된 seq. 아무도 입장하지 않았으면 {@code 0}이 맞는 값이다
