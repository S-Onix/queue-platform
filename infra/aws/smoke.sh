#!/usr/bin/env bash
# AWS SUT 위에서 도는 스모크. 전 구간 1건 + 시간 데이터 검증 + 자원 실측.
#   ssh ubuntu@<sut> 'cd ~/queue-platform && ./infra/aws/smoke.sh'
set -euo pipefail
B=http://127.0.0.1:18080
E="smoke-$(date +%s)@test.com"
MY="docker exec br-mysql mysql -uqueueapp -pqueueapp1234 queue_platform -N -B"
...
echo "[2/5] 컨슈머 적재 대기 (Kafka → tokens)"
for i in $(seq 1 40); do
  [ "$($MY -e "SELECT COUNT(*) FROM tokens WHERE token_id='$TID'")" = "1" ] && break; sleep 3
done

echo "[3/5] 시간대 조건"
TZ_G=$($MY -e "SELECT @@global.time_zone")
...
for chk in "tenants|created_at|email='$E'" "tokens|issued_at|token_id='$TID'" \
  ...
  D=$($MY -e "SELECT IFNULL(ABS(TIMESTAMPDIFF(SECOND, $C, UTC_TIMESTAMP(3))),'NULL') FROM $T WHERE $W")
  if [ "$D" = "NULL" ]; then fail "$T.$C 가 NULL"
  elif [ "$D" -gt 600 ]; then fail "$T.$C 가 UTC 와 ${D}초 차이 (32400 이면 KST 오적재)"
  else echo "  ✅ $T.$C — UTC 기준 ${D}초 이내"; fi
done
echo "  완료 상태: status=$($MY -e "SELECT status FROM tokens WHERE token_id='$TID'") (기대 2)"

echo "[5/5] 자원 실측 (16GB 안에 들어가는가)"
free -m | awk 'NR<=2'
