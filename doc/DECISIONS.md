...
> ⚠️ **§86에서 주기가 바뀌었다: 매일 00:30 → 매월 1일 00:05 (UTC).** 아래 "왜 월 1회가 아니라
> 매일인가"의 세 근거 중 둘(테넌트가 당월 사용량을 오늘 본다 / 한 번 실패해도 다음 날이 가져간다)은
> **의도적으로 포기했다.** 마감이 월 단위이고, 월말 23:59에 마감하면 그 달 마지막 60초가
> 누락되기 때문이다. 이 절은 당시 판단의 기록으로 남긴다.
...
마감은 월말이다. 그런데 **월말 23:59에 돌리면 그 달의 마지막 60초를 못 센다** —
`23:59:00~23:59:59.999`에 enqueue된 토큰이 집계 밖에 남고, 다음 마감은 한 달 뒤라
**청구서가 나간 뒤에야** 잡힌다. 경계를 완전히 넘긴 직후에 마감해야 누락 창이 0이 된다.

과금 기준이 **enqueue**이므로(§82) 월 경계에 걸친 큐는 토큰이 갈려 **앞뒤 달에 각각 청구**된다.
만료 여부는 보지 않으므로 이 분할에 상태가 개입하지 않는다.

🔴 **월 1회라 재시도 여유가 없다.** 매일 돌던 때는 실패해도 다음 날이 가져갔지만 이제 다음 기회가
한 달 뒤다. `queue_billing_snapshot_total{result="failure"}`가 **경보 대상**이다. 완전히 놓쳐도
마지막 방어는 남는다 — 그 달 파티션을 DROP하기 직전의 재집계다.

### 만료 사유를 싣는다 (`ExpiredReason`)

**테넌트는 넷 중 하나도 관여하지 않는다** — 전부 Platform이 자기 시계와 자기 키로 판정한 것이다.
테넌트만 아는 것은 따로 있지만(유저가 **왜** 폴링을 멈췄나) 우리는 그걸 알 필요가 없다.
`INACTIVE`가 기록하는 건 동기가 아니라 **"그 시간 동안 폴링이 안 왔다"는 관측 사실**이다.

**사유는 발행 지점에서 이미 갈려 있었다.** `publishExpired`를 부르는 메서드가 셋으로 나뉘어
있고(`reclaimExpiredWaiting`·`reclaimInactive`·`reclaimExpiredAdmits`), 어느 규칙이 발화했는지
알면서 부른다. **그걸 한 줄 뒤에 버리고 있었다.** 파급도 작았다 — 운영 코드 6곳 중 값을 채우는
곳은 1곳이고 나머지는 `admitToken`·`admittedAt`처럼 `null`을 넘긴다.

| 코드 | 사유 | 기록 주체 | 뜻 |
|---|---|---|---|
| 1 | `ADMIT_TTL` | **DB에 안 남는다** | admitToken 60초 만료 |
| 2 | `ADMIT_STALE` | `ReconcileJob` 직접 UPDATE | complete 창 300초 초과 → **Tenant 귀책** |
| 3 | `INACTIVE` | 컨슈머 | 폴링 끊김 → **정상 이탈** |
| 4 | `WAITING_TTL` | 컨슈머 | 7200초 초과 → **용량 부족** |

**🔴 1은 DB에 도달하지 않는다.** 컨슈머 가드가 `IF(status = 0, 4, status)`라 `ADMIT_ISSUED(1)`에서
통째로 no-op이고, 그 가드는 늦은 입장을 살리려고 일부러 넣은 것이다(§36). 같은 사람이 300초 뒤
`ReconcileJob`에 의해 `ADMIT_STALE(2)`로 기록되므로 정보는 잃지 않는다.

**🔴 사유에도 같은 status 가드가 필요하다.** 무조건 쓰면 `status = 1`로 살아 있다가 나중에
complete되는 토큰에 "만료됨" 사유가 박혀, `status = 2`인데 `expired_reason`이 채워진
**거짓말하는 행**이 생긴다.

🪤 **값을 `?`가 아니라 `new.expired_reason`으로 받는다.** ODKU의 SET 절에 `?`를 쓰면
`rewriteBatchedStatements`가 **조용히 꺼진다**(`admit_token`이 같은 우회를 하는 그 이유).

🪤 **`insertable = false`는 유지했다.** "컬럼이 있는데 쓸 수조차 없다"는 지적은 절반만 맞다 —
막히는 건 JPA 경로뿐이고 사유를 쓰는 `EXPIRED` 전이는 raw JDBC를 탄다. 실제로 풀었더니
`@SQLInsert`의 고정 컬럼 수와 어긋나 **`Parameter index out of range (8 > 7)`**로 11건이 깨졌다.

🪤 **집계는 `SUM(expired_reason <=> N)`이다.** `=`로 쓰면 전 행이 NULL인 그룹에서 SUM이 NULL을
돌려주고 `NOT NULL` 컬럼 적재가 통째로 실패한다(실측 8건). `status`에 같은 문제가 없는 건
`NOT NULL`이라서다.

🪤 **셋의 합은 `total_expired`와 다르다.** 사유를 남기기 전에 만료된 행이 NULL이다.
억지로 맞추지 마라 — 그 차이가 "언제부터 사유를 남기기 시작했나"다.
