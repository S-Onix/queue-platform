...
## §87 — 큐 상한 둘: `maxCapacity` 30만 + 테넌트당 큐 20개 (2026-09-03)

### 배경 — 근거가 없어서 미뤄뒀던 결정이다

`QueueCreateRequest`는 `@Min(1)`만 있었고 주석이 이유를 적어 놨다: *"maxCapacity 상한은 Redis
노드 메모리와 묶여 있고(§75 D25의 50% 임계), 그 값이 아직 정해지지 않았다. 근거 없는 숫자를
지금 박으면 나중에 두 번 정한다."* 그 유보는 **옳았다.** 이번에 그 숫자를 실측해서 유보가 풀렸다.

### 실측 ① — 대기자 1명당 Redis 사용량

슬롯 소유자 노드에 직접 붙어 실제 키 3종(`waiting` ZSet · `tokens` Hash · `last-active` ZSet)을
실제 값 형식(identifier UUID, `tokenId|issuedAt`)으로 적재하고 `used_memory` 차이를 쟀다.

```
N=200,000   delta  89,539,344 B   →  447 B/명
N=300,000   delta 143,241,176 B   →  477 B/명   (= 136 MB, 외삽 아님)
```

### 실측 ② — 안 두면 무엇이 깨지는가 (결함 주입)

같은 마스터에 붙는 큐 두 개(victim / greedy)를 잡고 `maxmemory`를 임시 축소한 뒤,
상한 없는 큐가 쏟아붓게 했다.

```
사전   victim enqueue 정상 (HSETNX 1 · ZADD 1)
주입   greedy ZCARD 166,372에서 마스터 포화
판정   victim(다른 테넌트) enqueue → OOM command not allowed
       victim 폴링(ZCARD)         → 정상
```

🔑 **상한이 지키는 것은 자기 큐가 아니라 같은 마스터에 sticky로 묶인 남의 큐다.** 그리고
**쓰기만 죽고 읽기는 산다** — 대기자에게는 "줄은 보이는데 못 들어가는" 상태로 나타난다.

### 실측 ③ — 그때 막아 세울 코드가 없다

`RedisClusterAssigner.assign()`은 1 아니면 2를 반환할 뿐 **거절 경로가 없다**(코드 확인).
cluster1이 50%를 넘으면 cluster2로 보내고, **cluster2가 꽉 차도 계속 cluster2로 보낸다**.
D29 단조 가드 때문에 되돌아오지도 않는다. 즉 무한 생성의 종착점이 위 OOM이다.

### 실측 ④ — 큐 개수 상한이 필요한 이유는 성능이 아니다

| 후보 근거 | 실측 | 판정 |
|---|---|---|
| 드레인 p99 | `BatchProcessor`는 `groupByQueueId(drained)` — 그 틱에 유입이 있는 **활성** 큐만 돈다 | ❌ 유휴 큐 비용 0 |
| `queues` 전량 SELECT | 100/1,000/10,000행 = 0.29 / 0.59 / 5.19 ms (서버), 10k 왕복 7.75 ms | ❌ 안 깨진다 |
| 파티션 프루닝 실패 | 큐당 0.14 ms (24 파티션 다이브 고정비) | ❌ §83의 "3%" 그대로 |
| enqueue 핫패스 | `findQueueAndVerifyOwner`는 `type: const`, `rows: 1` — 큐 수와 무관 | ❌ |
| **Redis 마스터 용량** | 아래 | ✅ **유일한 근거** |

🔑 **`maxCapacity` 상한 30만은 큐 하나만 막고 마스터 합계를 아무도 안 막는다.** 즉 개수 상한이
없으면 용량 상한이 개수로 우회된다.

```
477 B/명 → 마스터(maxmemory 4GB) 약 900만 명
D27-3의 50% 임계 = 450만 명 = 30만짜리 큐 15개
큐→마스터 분포는 균등  (keyslot 1,000개 실측: 25.1 / 25.0 / 24.9 / 25.0 %)
  ⇒ 전체 큐 60개면 한 마스터가 15개를 받아 임계 붕괴 · 120개면 OOM
목표 전제는 마스터당 2개 = 전체 8개 (FRS 규모 전제, 2026-09-02)
```

### Decision

- **`maxCapacity` ∈ [1, 300,000]** — `QueueCreateRequest`의 `@Max` 한 줄. 전용 검증 클래스 없음
- **테넌트당 큐 20개** — `QueueService.createQueue`의 `COUNT` 한 줄. `DELETED`는 세지 않는다
- 새 테이블·새 키·플랜 분기 **0개**

### Rationale

- **값 300,000**: 물리 상한 234만이 아니라 **문서화된 필요**(FRS "큐당 최대 30만")를 따른다.
  올리는 것은 하위호환이고 내리는 것은 파괴적 변경이라 "견딜 수 있는 최대"가 아니라
  **"필요를 채우는 최소"**에서 시작한다 — §80 ⑦의 `count` 상한 100과 같은 기준이다
- **값 20**: 모든 테넌트가 한도를 꽉 채워도 3개 테넌트까지 50% 임계 안에 산다.
  목표 전제(8개)의 2.5배라 미래 이벤트를 미리 만들어 두는 운용을 막지 않는다
- **DELETED 제외**: 상한의 목적이 Redis 용량인데 삭제된 큐의 키는 `waitingTtl` 뒤 회수 배치가
  비운다. 세면 큐를 지우고 **다른 이름으로** 다시 만드는 정상 운용이 영구히 막힌다.
  ⚠️ **같은 이름으로 다시 만드는 것은 이 결정과 무관하게 이미 막혀 있다** —
  `uq_queues_tenant_name (tenant_id, name)`에 status 조건이 없다
- **플랜별 기각**: `Tenant`가 `Plan.ENTERPRISE` 하드코딩이라 4분기 중 3개가 도달 불가능한
  죽은 코드가 된다(§4-1). 🔑 **의존성 방향은 반대다** — 상한은 상수로 완결되고, 플랜 변경 API가
  생기면 그때 `Plan`에 필드를 더하면 된다(상수 → 플랜값은 하위호환). **릴리스 게이트에서 두
  항목을 묶지 마라**

### Consequences

- ⚠️ **이 상한들은 메모리 보증이 아니라 blast radius 상한이다.** `maxCapacity`가 막는 것은
  `ZCARD waiting`뿐이라 admit된 사람의 `tokens` Hash 항목은 상한 밖에서 남는다(초과분은
  admit rate × TTL로 유계). **한 마스터의 합계는 여전히 아무도 안 막는다** — 개수 상한이
  그것을 *간접적으로* 유계화할 뿐이다. 노드 단위 admission control은 별건이다
- 🪤 **동시 생성의 초과분 상계는 "한두 개"가 아니라 동시 요청 수다.** `COUNT`↔`INSERT`가
  원자적이 아닐 뿐 아니라 **`createQueue`가 package-private이라 `@Transactional`이 아예 안 걸린다**
  (실측: `AnnotationTransactionAttributeSource`가 NULL. 기본 `publicMethodsOnly=true`.
  `updateQueue`·`pauseQueue`·`deleteQueue` 셋은 걸린다 — **이 메서드만 예외다**).
  `exists`·`count`·`save`가 각각 다른 autocommit 커넥션에서 돈다. 큐 0개인 테넌트가 서로 다른
  이름으로 50개를 동시에 쏘면 **50개 전부 통과**한다(이름 UNIQUE는 이름이 다르면 안 걸린다).
  IaC 병렬 생성이 정확히 그 형태다.
  잠그지 않는다: 임계가 60개인데 상한이 20이라 여유가 3배고, "테넌트당 N행"은 DB 제약으로
  표현할 수 없으며(UNIQUE로 하려면 slot 컬럼·마이그레이션·재사용 로직), 막는 대상이 악의가
  아니라 오타다. ⚠️ **`createQueue`의 가시성은 이 커밋 범위 밖의 선재 결함이다**(dev `f146dce`에도
  package-private). 별건으로 남긴다
- 🟠 **`createQueue`의 `@Transactional`이 안 걸린다 (선재 결함, 별건).** 이 메서드만
  package-private이라 Spring이 트랜잭션 속성을 NULL로 준다(실측. `updateQueue`·`pauseQueue`·
  `deleteQueue` 셋은 걸린다). dev `f146dce`에도 있던 것이라 이 커밋에서 고치지 않는다 —
  가시성 한 단어로 `exists`·`count`·`save`의 트랜잭션 경계가 바뀌므로 별도로 판단한다.
  🪤 그 탓에 `QueueJpaRepository` 주석의 "선언한 파생 쿼리라 master"라는 근거가 **우연이었다** —
  진짜 이유는 호출자가 트랜잭션을 안 열기 때문이다. 결론은 같지만 근거를 고쳤다.
  ⚠️ 이 COUNT를 `readOnly = true` 메서드에서 재사용하면 **replica로 가고 상한이 조용히 뚫린다**

  > ✏️ **해소됨 (2026-09-03, `fix/create-queue-no-transaction`).** 가시성을 바꾸는 대신
  > **애노테이션을 뗐다** — public으로 살리면 셋 다 손해였다: 쓰기가 `save` 하나뿐이라 원자성을
  > 못 사고, `REPEATABLE-READ`의 비잠금 `SELECT COUNT`는 갭 락을 안 잡아 경쟁도 못 막으며,
  > `RedisClusterAssigner.assign()`의 `INFO` 왕복이 DB 트랜잭션 안으로 들어온다.
  > 런타임 동작은 안 바뀐다(원래 안 걸렸다). 위 두 항목의 "package-private이라 안 걸린다"는
  > **그 시점의 서술**이다 — 지금은 애노테이션 자체가 없다. `QueueServiceTest`가 효력(NULL)과
  > 표기(`isAnnotationPresent`)를 **둘 다** 잠근다. 효력만 잠그면 되붙여도 통과한다(실측).
  > ⚠️ replica 경고는 **여전히 유효하다** — 트랜잭션이 없어서 master인 것이지 안전해진 게 아니다.
- 🟠 **REST로 21번째 큐를 만들어 409를 확인하는 전 경로 테스트가 없다.** 서비스 로직(목)과
  어댑터 카운트(실 DB)가 따로 잠겨 있고, Spring이 둘을 물려주는 조립만 미검증이다. 생성자 주입
  하나라 깨질 여지가 낮아 안 만들었다(만들면 큐 20개 선적재 + 공유 테넌트 정리로 diff 3배)
- `idx_queues_tenant_status(tenant_id, status)`가 **첫 소비자를 얻었다.** 2026-09-03까지 이
  인덱스를 쓰는 쿼리가 0건이었다(§4-1 후보였다)
- `QueueCreateRequest`의 "상한은 두지 않는다" 문단은 **폐기**하고 갈아엎었다 — 덧붙이지 않았다.
  근거가 소멸한 유보를 남겨두면 다음 사람이 그것을 믿고 되돌린다

### 🔴 상한은 API 검증이다 — 운영 우회로가 있다

`doc/monitoring/runbook/enqueue.md:254`와 `doc/monitoring/queries/enqueue.md:211`이 FULL 대응으로
**`UPDATE queues SET max_capacity = <새값>`을 직접 지시한다.** 상한은 DTO에만 있으므로
**운영자가 런북대로 하면 30만을 넘긴다.** 그 값은 드레인 용량 캐시가 30초 안에 읽어가고
(§PR #66), Lua 가드가 그 값에서 열린다 — 위에서 막으려던 마스터 포화가 그대로 재현된다.

새 방어 장치는 만들지 않는다(운영자는 신뢰 경계 안이고, 막으면 장애 대응 수단이 사라진다).
대신 런북 두 곳에 "이 UPDATE는 API 상한을 우회한다"를 명시했다.

🔑 **PR #66에서 이미 한 번 당한 패턴이고 또 당했다** — `grep`을 코드에만 돌렸다.
**§4-2의 전수 범위에 런북·쿼리집이 들어간다는 것을 세 번째로 확인한 셈이다.**

### 남긴 것 (§4 — 지금 안 만든다)

- **배치 순회에 종료 조건이 없다.** `TokenReclaimJob`(10초)·`ReconcileJob`(5분)이
  `queueRepository.findAll()`로 **DELETED 큐까지** 영원히 순회한다. 이건 의도된 설계이고
  (`QueueRepository.findAll` 주석 — 소프트 삭제라 Redis 키가 남아 순회해야 한다) **필터를
  걸면 그 키가 noeviction에 영구 잔류**하므로 필터가 답이 아니다. 올바른 형태는 "정리가
  끝났는가"라는 **종료 조건**(`deleted_at < NOW() - (waiting_ttl + 여유)`)이다.
  지금은 아무것도 안 깨진다(현재 `queues` 20행, DELETED 0행) → 트리거: `queues` 1,000행 또는
  reclaim 한 바퀴가 주기의 절반 초과
- **`ReconcileJob`의 진짜 포화점은 큐 개수가 아니다.** `findSettledMaxSeq`가 `status` 술어 없이
  큐의 토큰 **전량을 스캔**한다(7,279행 = 8.0ms, **1.1 µs/행** 실측). 포화는
  `Σ(전 큐의 토큰 수) × 1.1µs > 300초` = **누적 2.7억 행**(여유 20%면 5,400만).
  🔑 **잴 수단이 지금 0이다** — 잡에 벽시계 타이머가 없다. **인덱스보다 타이머 메트릭이 먼저다**

### 검증

| 항목 | 방법 | 결과 |
|---|---|---|
| 447 / 477 B/명 | 슬롯 소유자 직접 적재 2판 | ✅ 실측 |
| co-tenant OOM | 결함 주입(`maxmemory` 임시 축소, 원복 확인) | ✅ 재현 |
| 큐→마스터 균등 | keyslot 1,000개 해시 | ✅ 25.1/25.0/24.9/25.0% |
| `assign()` 거절 경로 0 | 코드 전문 | ✅ |
| 우회 경로 (API) | `createQueue` 호출자 1곳 · `QueueUpdateRequest`는 `name`만 | ✅ 없음 |
| 우회 경로 (운영) | 런북이 `UPDATE queues SET max_capacity` 를 지시한다 | 🔴 **있다** (아래) |
| 전체 회귀 | `./gradlew test` | ✅ **현재 459건 실패 0**(skipped 4) · 단위 287건 |

**결함 주입 6종** (tester 보강 전/후를 각각 전량 실행. ⚠️ 앞 셋은 **처음엔 안 잡혔다**)

| 변이 | 보강 전 | 보강 후 | 무엇을 뜻하나 |
|---|---|---|---|
| `@Max` 제거 | ✅ 1 FAILED | ✅ | |
| 경계 `>=` → `>` | ✅ 1 FAILED | ✅ | |
| `@Max(300_000)` → `@Max(3_000_000)` | ✅ 1 FAILED | ✅ | 300001이 값을 잠근다 |
| `MAX_QUEUES_PER_TENANT` 20 → 999999 | 🔴 **0 FAILED** | ✅ 1 FAILED | 스텁이 **상수 자신**을 참조해 같이 따라갔다 → 리터럴 20/19로 교체 |
| 어댑터가 DELETED **포함**(`countByTenantId`) | 🔴 **0 FAILED** | ✅ 1 FAILED | 파생 쿼리가 **어떤 테스트에서도 SQL로 안 돌았다** |
| 어댑터가 **항상 0** 반환 | 🔴 **0 FAILED** | ✅ 1 FAILED | **상한이 프로덕션에서 무동작인데 전 스위트 초록**이었다 |

🔑 **가장 위험한 것이 가장 조용했다.** 상한이 아예 안 도는 상태가 458건 전부 초록이다.
원인은 하나다 — `QueueService`는 포트를 목으로 끊고, `QueueJpaAdapterTest`는 `JpaRepository`를
목으로 끊는다. **어댑터의 파생 쿼리가 실 DB에서 실행되는 지점이 0곳이었다.**
🪤 메서드명 오타는 컨텍스트 기동이 잡지만(통합 레인에서 6건 initializationError),
**이름이 맞고 세는 기준이 틀린 것은 기동으로 안 잡힌다.**
→ `QueueLifecycleContractTest`(`@Tag("mysql")`)에 `countActiveByTenantId` 증분 단정 1건을 넣어 닫았다.

🪤 **단위 레인(`-PunitOnly` 287건)만으로는 이 기능의 DB 계약이 하나도 검증되지 않는다** —
같은 오타 주입이 통합 레인에서는 6건을 죽이는데 단위 레인은 287건 전부 초록이다(실측).
...
## §89 — 테넌트 rate limit을 100,000 → 50,000으로 내린다 (버킷은 테넌트 단위 유지)

### Context

§88이 등급제를 걷어내면서 한도를 상수 둘로 남기고 **값 재조정은 후속으로 미뤘다.** 그 상수를
k6로 재봤더니 두 가지가 드러났다.

**① §88이 근거로 쓴 비교가 모집단이 어긋나 있었다.** §88은 `sweep.js`(enqueue 전용)의 1,600 rps를
`refill` 1,666.67과 견줬다. 그런데 테넌트 버킷은 enqueue 전용이 아니라 **인증된 요청 전부**가
공유한다(`RateLimitFilter:178`). 전 구간 부하로 실측한 소비량은 이렇다.
...
| verify | 124.63 | |
| complete | 124.63 | |
| admit | 6.23 | `ADMIT_N=20` |
| **합(테넌트 버킷)** | **382.46** | **enqueue의 3.01배** |
| poll | 124.63 | 별도 버킷 `rl:poll:token:{tid}` — 안 먹는다 |
| `/status` | 268.16 | 한도 없음 |

즉 **유저 1명이 생애 전체로 3.01 토큰**을 먹는다. enqueue rps만 보면 소비를 3배 과소평가한다.

**② 지금 값에서 리미터가 한 건도 작동하지 않는다.** 다른 테넌트가 3,000 rps × 30초(90,000건)를
밀어넣는 판에서 **429가 0건**이었다. `capacity` 100,000 안에서 전부 끝난다.

### Decision

**`TENANT_CAPACITY` 100,000 → 50,000, `TENANT_REFILL_PER_SEC` 1,666.67 → 833.34.**
비율 `capacity = refill × 60`(§62)은 유지한다. **버킷 키는 `rl:tenant:{id}` 그대로다.**

🔑 **버스트 순간의 비용은 3.01이 아니라 1이다.** verify·complete는 몇 분 뒤에 오므로, 몰리는
순간 버킷을 먹는 것은 enqueue뿐이다. 그래서 "동시 N명이 통과하나"는 `capacity`가 혼자 정한다 —
50,000이면 **동시 5만 명까지 0초에 통과**한다. 1만 명 급 이벤트는 이 값에 닿지도 않는다.
지속 수용은 다르다: 833.34 ÷ 3.01 = **초당 277명**(완주율 100% 기준, 이탈이 많으면 더 늘어난다).

### 실측 (2026-09-04, 앱 3종 실기동 · 하니스 설정 고정)

피해 테넌트 200 rps 정상 부하, 공격 테넌트(**다른 apiKey = 다른 버킷**) 30\~60초에 3,000 rps.

| | capacity 100,000 | **capacity 50,000** |
|---|---|---|
| 공격자 429 | **0건** | **12,819건** |
| 공격자 200 | 89,980 | 76,866 |
| 피해자 429 · 503 | 0 · 0 | 0 · 0 |
| 피해자 p99 (pre / attack / post) | 30.69 / 46.4 / 43.03 ms | 32.02 / 69.16 / 33.18 ms |

**허용 총량이 이론과 맞는다**: 예측 `50,000 + 833.34×30 = 75,000`, 실측 **76,866** (+2.5%).

⚠️ **피해자 p99의 attack 구간 차(46.4 → 69.16ms)는 판정하지 않는다.** 각 1판뿐이고, 이 레포의
k6 측정은 순서·워밍업만으로 2배가 흔들린 전례가 있다(드레인 주기 재조정 건). **미검증**이다.
robust한 것은 **429 건수(0 → 12,819)와 허용 총량이 이론과 일치한다는 것** 둘뿐이다.

### Alternatives

| 안 | 내용 | 판정 |
|---|---|---|
| ㉠ 테넌트 버킷 유지 | 큐를 몇 개 만들든 총량 고정 | ✅ **채택** |
| ㉡ 큐 수에 비례 | `capacity = base × min(큐수, k)` | 기각 — 상한 `k`가 필요하니 결국 ㉠에 계단 하나 |
| ㉢ 큐당 버킷 (`rl:queue:{id}`) | 큐마다 독립 | 🔴 기각 — **§87과 같은 이유로 개수로 우회된다.** 큐 20개 만들면 한도가 20배 |
| 값을 올린다 | refill 상향 | 기각 — "안 올리면 무엇이 깨지나"에 답이 없다(§4). 1만 명 버스트는 현행으로 0초 통과 |

**㉠의 대가**: 테넌트가 §87 상한인 큐 20개를 만들면 큐당 지속 **14명/s**다. 20개를 동시에 최대
부하로 돌리는 시나리오는 rate limit 이전에 Redis 쪽에서 먼저 걸린다(동시 오픈 8개가 실측 상한).

### 🪤 함정 — refill을 833.33으로 쓰면 안 된다

`50000/833.33 = 60.0002` → `token-bucket.lua`의 `ceil()`이 **61**로 올라가 버킷 TTL이
120초 → 121초로 어긋난다. 비율 단정이 `within(1)`이라 **테스트는 둘 다 통과한다.**
나눗셈이 정확히 60 이하로 떨어지는 값(833.34)을 써야 한다.

### 올려도 안 되는 것 / 안전한 것 (코드 확인)

**유입에 비례하지 않는다** — Redis enqueue 호출(20ms 배칭, `BatchProcessor:384`로 초당 50회
...
- ~~(e)의 404는 코드 경로 추적이다~~ → ✅ **실측 완료 (2026-09-11, 컨테이너 prod 프로필 api 3대).**
  컨슈머를 멈춰 DB 적재가 없는 구간을 만들고, ①안을 **재빌드 없이** 모사했다 — verify 직후
  `admit-by-admit` 키를 직접 `DEL`하면 ①안이 만들었을 Redis 상태와 같다.

  | | verify | 키 삭제 | complete |
  |---|---|---|---|
  | B-1 대조군 (현행 ④안) | 200 | — | **200** (`complete가 Redis 폴백으로 처리됐다` WARN 확인) |
  | B-2 실험군 (①안 모사) | 200 | 1건 | **404 TK002** |

  컨슈머 재기동 후 토큰 수가 3 → 5로 늘어 **그 구간에 DB 행이 실제로 없었음**도 함께 확인했다.
...
## §94 — enqueue 입구에 Kafka를 두지 않는다: `Redis → Kafka → DB` 유지 (2026-09-19)

### Context

제안: **`Redis → Kafka → DB`를 `Kafka → Redis → DB`로 바꾸고, `RedisQueueEngine.globalQueue`
(프로세스 메모리)를 Kafka가 대신한다.** 기대 이득 둘 — ①요청 순서 보장 ②프로세스 사망 시 유실 창 제거.

3인 검토(architect · backend · monitoring) **전원 기각.** 아래는 그 근거이고, 같은 제안이
다시 올 때 이 절을 먼저 읽으면 된다.

### Decision

**순서를 유지한다.** enqueue 입구에 Kafka를 두지 않고 `globalQueue` + 20ms 드레인을 그대로 둔다.
§70 D9(`INCR` 순번) · §71 D11(Redis 먼저) · §73 D16(`key=tokenId`) · D18(단일 토픽)을 **뒤집지 않는다.**

### Rationale

**① 기대 이득 ①은 얻어지지 않는다.** "요청 순서"에 세 가지가 섞여 있다:

| 종류 | 현재 | 제안으로 |
|---|---|---|
| 같은 토큰의 상태 전이 순서 | ✅ 보장(§73 D16) | 유지 |
| 큐 내 순번의 유일성·단조성 | ✅ 완전 보장(`INCR`, 1,000 동시 0~999 유일 실측) | 유지 |
| **도착 순서 ≡ seq 순서** | ❌ 미보장 | **여전히 미보장** |

제안이 사려는 것은 셋째 하나인데, `key=tokenId`를 유지하면 파티션 인터리빙 때문에 **오히려
나빠지고**(파티션 랙 편차 > 20ms 틱), `key=queueId`로 바꾸면 §73 D16이 "한 큐 30만이면 99%가
한 파티션"으로 기각한 값이 돌아온다. 그리고 **파티션 수·키는 되돌릴 수 없다**(D17).
🔑 한 토픽은 키가 하나다 — 첫째는 `tokenId`를, 셋째는 `queueId`를 요구하므로 **동시 만족이 불가능**하다.
🔑 그리고 **셋째를 요구하는 계약이 아무 데도 없다.** 20ms 창 안의 도착 순서는 사용자에게 관측 불가다.

**② 제안은 자기 기각 논리를 상속한다 (가장 결정적).**
§71 D11이 "왜 DB auto-increment로 순번을 못 매기나"에 이미 적어 뒀다 —
> **순서** — DB auto-inc = insert 순서(**Kafka 파티션 병렬·Consumer 배치**) ≠ **도착 순서**

제안은 Redis `INCR`을 **바로 그 자리(Kafka 뒤)로 옮긴다.** 남는 정당화는 "큐별 스코프" 하나인데
그건 DB에서도 만들 수 있다.

**③ 기대 이득 ②는 제거할 유실이 없다.** `globalQueue`에 있다가 죽는 항목은 Redis·Kafka·DB
**3자 전부에 흔적이 없고** 클라이언트는 503을 받는다 — 데이터 유실이 아니라 **실패한 요청**이고
재시도가 `HSETNX`로 멱등이다. Kafka를 앞에 두면 그 자리에 **"Kafka엔 있는데 클라이언트는 실패를 본"
좀비**가 들어온다(§82 구멍 ③과 같은 모양, 청소는 `waitingTtl`이 최대 2시간 뒤).

**④ `seq` 동기 반환이 못 박힌 계약이다.** `EnqueueResponse`가 `seq`·`rank`·`total`을 동기로 주고,
폴링이 `?seq=`를 **필수**로 받아 `poll_verify.lua`가 그 값으로 찾는다.
`doc/TENANT_INTEGRATION.md`가 "**`seq`를 뺄 수 없다**"고 명시한다. 빼면 폴링을 시작할 수 없다.
대안(reply 토픽 + correlation id)은 Kafka 왕복이 1회 → 3회가 되고, **발행 한 조각만 AWS 67~128ms**
인데 현행 **전 구간 p99가 32.32ms**다. 부품 하나가 예산의 2~4배다.

**⑤ 리밸런스가 평시 장애가 된다.** 지금은 적재만 멈추고 대기열·폴링·admit이 정상이다.
제안에선 **줄 자체가 멈춘다** — 신규 진입자가 순번을 못 받아 폴링을 시작할 수 없다.
리밸런스는 **배포마다 일어나는 평시 이벤트**다.

**⑥ 같은 랙이 의미를 바꾼다.** AWS 8차의 LAG 695만(54분)은 지금 **원장만** 늦다.
제안에선 그 54분이 "줄에 존재하지 않는 시간"이 된다.

**⑦ enqueue 실효 천장이 내려앉는다.** 9,922/s(미포화, k6) → 컨슈머 배출율(1,400~1,800/s).
그리고 현 구조의 좋은 성질을 잃는다 — 지금은 틱마다 배칭돼 **Redis 마스터 부하가 유입률과 무관**하다.

**⑧ 대사가 항등식이 된다.** `ReconcileJob`이 증거인 이유는 Redis 경로(api)와 DB 경로(consumer)가
**독립**이라서다. 한 컨슈머가 둘 다 쓰면 같은 출처의 파생값 두 개를 비교하게 되고,
**발행 유실 탐지 능력이 사라진다** — 초록은 유지되고 신호만 죽는다(§4-1이 반복 지적한 모양).

**⑨ admit이 Redis만 본다(§80).** Kafka 랙이 곧 "200 받았는데 `waiting` ZSet에 없는 사람"이 되고
admit이 그를 건너뛴다 → **순서를 사려는 변경이 순서를 더 크게 깬다.**

### Consequences

- "도착 순서 ≡ seq 순서"는 앞으로도 보장하지 않는다. 요구하는 계약이 없다
- `globalQueue`의 종료 중 유실은 남지만 클라이언트에겐 **실패로 보이고** 재시도가 멱등이다
- **새로 만드는 것은 0이다**(§4)
- ⏳ **선행 과제 하나만 남는다** — `Shutdown drain incomplete` 로그가 실제 배포에서 **몇 건 찍히는가.**
  아무도 안 쟀다(로컬 0건). **0에 가깝다면 이 제안의 동기 자체가 사라진다.**
  다음 AWS 롤링 배포에서 측정한다
- 🪤 검토 중 `architect`가 `CLAUDE.md`에 없는 문장을 인용했다("비동기를 끼워놓고 동기 응답을
  약속하기"). 논지는 맞았지만 **근거가 지어낸 것**이라 판정에서 제외했다 — 보고는 대조해야 한다

### 부수 발견 (제안과 무관, 같은 검토에서 나옴)

- `doc/monitoring/runbook/enqueue.md` 유실 상한 공식이 `× 1초`로 **50배 과대**였다
  (드레인 1,000ms 시절 잔재) → `× 0.02초`로 정정
- 같은 런북의 "관측 불가다"가 낡았다 → `queue_pending_size`가 생겨 **"쌓이는 중"은 보인다.**
...
```java
     ...
     *    🔧(2026-09-25 정정) 거짓이었다 — 큐 삭제 시 purgeDeleted 가 seq 키를 지운다.
