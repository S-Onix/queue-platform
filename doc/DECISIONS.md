...
🪤 **그런데 `CREATE TABLE`은 백틱 없이 통과한다.** 컬럼 *정의* 자리에서는 예약어가 허용되기
때문이다. 그래서 테이블은 4개월간 멀쩡히 존재했고, 스키마만 봐서는 이 결함이 **보이지 않았다.**
"DDL이 성공했으니 DML도 되겠지"가 깨지는 자리다.
...
둘째로 **스키마에 4개월간 있던 예제 쿼리가 실행하면 죽는 것이었습니다.** 컬럼명이 `year_month`인데
그게 MySQL 예약어예요. 그런데 `CREATE TABLE`은 백틱 없이도 통과합니다 — 컬럼 정의 자리에서는
허용되거든요. 그래서 테이블은 멀쩡히 존재하는데 그걸 읽는 문장만 죽습니다. **DDL이 성공했다는 게
DML이 된다는 뜻이 아니라는 걸** 여기서 배웠습니다.
