...
- **만료 사유는 구분되지 않는다.** `total_expired` 160,819건이 admitToken TTL인지 이탈인지
  `waitingTtl` 초과인지 알 수 없다. `TokenReclaimJob.publishExpired()`가 3경로 전부에 **동일
  이벤트**를 발행하고 `tokens.expired_reason`은 쓰는 코드 0건 + `insertable = false`다.
  셋의 의미는 **정반대**다(Tenant 귀책 / 정상 이탈 / 용량 부족) — 합치면 조치로 이어지지 않는
  "만료율 99.98%"만 남는다. 사유를 이벤트에 싣는 것은 별건으로 남긴다.
