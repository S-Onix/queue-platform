...
| 미정 항목 | 상태 |
|---|---|
| **큐에 종속되지 않는 키의 소속** | **미정 — 가장 시급.** `rl:*`, `apikey:*`, `tenant:*`, `refresh-token:*`은 특정 큐에 속하지 않아 **D26이 답을 주지 않는 유일한 부류**다. cluster1 고정이 단순하나 그러면 cluster1 부하가 안 줄어든다 |
...
> 매핑 저장 위치 → D27-1 / rebalancing → D27-2
...
> **클러스터·shard를 담을 컬럼이 없다.** 이 방식을 택하면 `queues`에 컬럼 추가 + `Queue` 도메인 필드 추가 + 생성 시 배정 로직이 필요하다. (컬럼 추가 여부는 D29 — 미정)

**④ 50% 임계는 "무엇을 재는 값"인지부터 정의해야 한다 (미정)**
같은 "50%"라도 아래에 따라 전환 시점이 크게 달라진다.
- master **4대 각각의** `used_memory`인가, 4대 **합계**인가 (한 대만 편중돼도 넘길 것인가)
- replica를 포함하는가
- `maxmemory` 대비 비율인가, 절대 바이트인가 (참고: `doc/reviews`의 Redis 설정 점검에서 `maxmemory 0` 권고가 나와 있다 — 0이면 "대비 비율"이 성립하지 않는다)
- 측정 주체는 누구이고 어디에 캐시하는가 (매 enqueue마다 `INFO`를 칠 수는 없다)
- 한 번 넘으면 되돌아오지 않는가(히스테리시스), 진동하면 큐가 두 클러스터에 번갈아 배정된다
...
**⑦ Rate Limit·캐시 키는 무변경이지만 "어느 클러스터에 두는가"가 새 질문이 된다**
...
- 한 클러스터 안에서는 문제 없다. 다만 **클러스터가 둘이 되면 "테넌트 한도 카운터가 어느 클러스터에 있는가"**가 생긴다. 큐와 달리 이 키들은 큐에 종속되지 않으므로 D26(큐 단위 라우팅)이 답을 주지 않는다. → 전환 설계 시 별도 판단 필요 (예: 한쪽 고정 / 양쪽 독립 카운터 허용)

**⑧ 연결 설정은 "빈 1개 → 클러스터 N개"로 성격이 바뀐다**
`RedisConfig.redisConnectionFactory()`는 지금 `RedisConnectionFactory` **단일 빈**이고, `StringRedisTemplate`도 그 하나를 받는다. 이중 라우팅은 **연결처가 여러 개**라는 뜻이므로 단일 빈 주입 구조 자체가 바뀐다(라우팅 계층 필요). yml 9개도 함께. Lettuce의 MOVED/ASK 리다이렉트 처리·`ClusterTopologyRefreshOptions`는 **현재 미설정**이며 전환 시 결정 항목이다.
...
- `doc/schema.sql` `queues` 테이블 (③ — 클러스터 컬럼 없음)
...
1. **큐에 종속되지 않는 키의 소속** — `rl:*`, `apikey:*`, `tenant:*`, `refresh-token:*`은 어느 클러스터에 두는가? cluster1 고정인가, 양쪽 독립 카운터를 허용하는가? (Consequences ⑦) ← **가장 시급. D26이 답을 주지 않는 유일한 부류다**
2. **50% 판정의 측정 주기·캐시 위치** — 기준은 D27-3에서 정해졌으나, 얼마나 자주 어디서 재는지는 미정
3. **임계 히스테리시스** — 50% 부근에서 진동하면 큐가 두 클러스터에 번갈아 배정된다. 되돌아오지 않게 할 것인가?
4. **클러스터 개수** — 2개 고정인가, N개로 늘어날 수 있는 구조로 설계하는가?
5. **전환 시점** — 무엇이 먼저 끝나야 착수 가능한가? (§66은 Sprint 10, `QueueKeys` 주석은 Sprint 12+로 적혀 있어 서로 다르다)
6. **프로덕션 노드 구성** — §69의 4×4×4GB가 확정인가, 검토안인가?
