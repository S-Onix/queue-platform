...
2. 0행이고 **이미 완료된 행**이면 → **처음 완료된 시각**을 그대로 돌려준다 (재시도 안전)
   - 🔑 이 분기는 **재시도를 위한 것이다.** `verify` 뒤에 `complete`를 습관적으로 붙이면
     정상 경로가 통째로 여기로 오는데, 그 비용이 `UPDATE` 0행 + 이 `SELECT`다. AWS 8차에서
     DB 쿼리 예산의 **35.9%** 였다 — 계약은 **둘 중 하나만** 부르는 것이다
     ([`TENANT_INTEGRATION.md`](TENANT_INTEGRATION.md) 계약 ①)
3. 0행이고 행 자체가 없으면 → **Redis 폴백**. 적재 지연 구간의 정상 입장자를 살린다 — **단 입장 후 60초까지만**(아래 🔴)
...
>
> 🔴 **그러나 폴백이 적재 지연 전체를 덮지는 않는다.** DB 창의 시작점 `admitted_at`은 **적재 시점**에
> MySQL이 찍는다(§90). 적재 지연을 L이라 하면 Redis 창은 입장 후 `[0, 60]`, DB 창은 `[L, L+300]`이다.
> **L > 60이면 `(60, L)` 구간의 `complete`는 `TK002` 404**이고, 적재가 끝나면 같은 요청이 200이다
> (2026-09-25 로컬 재현: 82초 404 → 101초 200. AWS 14차 이 404 308,502건). 그래서 이 404는 **일시적**이며,
> Tenant에는 "입장 후 300초까지 404면 재시도"로 안내한다([`TENANT_INTEGRATION.md`](TENANT_INTEGRATION.md) 계약 ⑤).
