...
- **감소하지 않는다** — admit 미구현이라 한 번 오르면 내려오지 않는다
...
- 앞 5명의 score(=seq)가 시간이 지나도 그대로 → 정상(admit 미구현)
...
| 지표 | 정상 | 이상 |
|---|---|---|
| p50 | 0.4~1.1s (배치 주기 1초의 필연) | > 2.0s → Redis/Kafka |
| p99 | ≤ 2.7s (로컬 WSL2 10만 건 실측) | > 5s → 적체 |
