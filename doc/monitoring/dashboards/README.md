...
**Kafka 반영 지연**(맨 아래 패널)은 같은 현상을 **초**로 본다 — lag 33만이 몇 초 늦은 것인지는 lag만으로 모른다.
60초 선을 넘으면 verify 폴백의 원장 창(반영 시각부터 60초)이 입장권 수명보다 늦게 열린다.
🪤 컨슈머를 껐다 켜면 밀린 이벤트가 실제로 늦은 만큼 찍혀 **p99가 상한(3600초)에 붙는다** — 결함이 아니라 재기동 직후의 정상 모습이다.
