...
### 선행 결정 (이게 안 정해지면 착수 불가)

| # | 결정할 것 | 왜 지금(=먼저)인가 |
|:-:|---|---|
| 1 | **큐 → 클러스터 매핑을 어디에 저장하는가** | 이게 없으면 어떤 코드도 "이 큐가 어느 클러스터인지" 물어볼 수 없다. `queues` 테이블에 **해당 컬럼이 없음**(schema.sql 확인) → 스키마 변경 여부가 여기서 갈린다 |
| 2 | **50% 임계의 정의** (master 개별/합계, replica 포함 여부, `maxmemory` 대비/절대값, 측정 주기, 히스테리시스) | 배정 로직의 입력값이다. 정의가 없으면 "언제 cluster2로 보내는가"를 구현할 수 없다 |
| 3 | **클러스터 개수** (2 고정인가, N 확장형인가) | 라우팅 계층을 2개 전용으로 짤지 목록 기반으로 짤지가 갈린다. 나중에 바꾸면 키 이름(shard 태그)까지 영향 |
| 4 | **rebalancing 여부** | "새 큐만 cluster2로"면 마이그레이션 작업이 **통째로 사라진다**. 아니면 이동 절차·정합성 검증이 통으로 추가된다 |
| 5 | **큐에 종속되지 않는 키의 소속** (`rl:*`, `apikey:*`, `tenant:*`, `refresh-token:*`) | 큐 단위 라우팅(D26)이 답을 주지 않는 유일한 부류다. 한도 카운터가 클러스터별로 쪼개지면 SLA가 2배로 샌다 |
| 6 | **shard 태그 전환 여부** (`queue:{queueId}:` → `queue:{shard_X}:{queueId}:`) | 키 이름이 바뀌므로 **기존 대기열 데이터가 새 키로 따라오지 않는다.** 되돌리기 어렵다 |
| 7 | **프로덕션 노드 구성** (§69의 4×4×4GB가 확정인가) | 2·3번과 맞물린다 |
| 8 | **전환 시점** (어느 Sprint) | 위가 정해져야 기간 산정이 가능 |

### 작업 (선행 결정 이후, 의존 순서)

| # | 작업 | 왜 이 순서인가 |
|:-:|---|---|
| 1 | 큐 → 클러스터 매핑 영속화 (스키마 + `Queue` 도메인 필드 + 조회 경로) | 라우팅의 **조회 키**. 이게 없으면 라우터가 물어볼 대상이 없다 |
| 2 | 큐 생성 시 클러스터 배정 로직 (용량 판정 + 배정 + 고정) | 매핑이 **써지는** 경로. 1번 다음 |
| 3 | Redis 연결 라우팅 계층 (`RedisConfig` 단일 빈 → 클러스터별 커넥션 + 큐 기준 선택) | 1·2가 있어야 "어디로 보낼지"를 결정할 수 있다. 현재 `RedisConnectionFactory`가 **단일 빈**이고 `StringRedisTemplate`이 그것 하나를 받는 구조라 주입 형태가 바뀐다 |
| 4 | `application*.yml` 9개 + `RedisConfig` Sentinel → Cluster 전환 | 3번 라우팅 계층이 요구하는 형태가 정해진 뒤에 설정을 맞춘다 |
| 5 | Lettuce Cluster 옵션 결정 (`ClusterTopologyRefreshOptions`, MOVED/ASK, `ReadFrom`) | 4번 위에서만 의미 있음. 현재 **전부 미설정** |
| 6 | 배치·스위퍼를 "DB에서 큐 목록 → 큐별 클러스터 지정 접근" 형태로 설계 | 3번 라우터가 있어야 큐별 접근이 가능. §73에서 `findDrainableQueueIds`가 제거돼 큐 목록 출처가 DB로 확정된 상태 |
| 7 | 다중 키 Lua 로컬 Cluster A 실증 (`enqueue_bulk` 3키, `poll_verify` 3키) | 4번 이후. **Sentinel 테스트로는 CROSSSLOT이 절대 안 잡힌다** |
| 8 | 이중 라우팅 통합 테스트 (로컬 A/B 동시 사용, 큐가 갈리는지 + 갈린 큐가 서로 안 섞이는지) | 마지막. 로컬에 A(7001-7008)/B(8001-8008)가 이미 있어 실증 가능 |
| 9 | 모니터링 보강 (`MONITORING_DESIGN.md` §2-2가 Sentinel 전제 — 클러스터별 `used_memory`·슬롯·리다이렉트 지표) | 2번의 50% 판정 근거를 실제로 관측하려면 필요. 판정 로직(2번) 이후 |

### 이번에 하지 않은 것 (미룬 이유)

- **운영 코드·yml 수정** — 이번 작업은 문서 드리프트 정리다. 전환 시점이 미정이므로 코드를 먼저 바꾸면 Sentinel 환경이 즉시 깨진다
- **Sprint 배정** — 사용자가 정할 사항 (선행 결정 8번)
