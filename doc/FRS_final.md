...
Kafka 발행 실패: 200을 준다. Lua가 이미 커밋됐고 되돌릴 수 없다.
  ...
  🔑 **전량 발행 뒤 한 번에 기다린다**(2026-09-23 변경, `publishAll`). 예전엔 건별 ack 을 기다리며
     **첫 실패에서 나머지를 건너뛰었다** — AWS 12차에서 300건 admit 이 p50 **1.797초**였고(건당
     5.99ms = linger 5ms + 왕복 1.38ms), 실패 1건이 **최대 299건의 원장까지** 데려갔다.
  🔴 **첫 실패에서 끊지 않는다.** Kafka 에 트랜잭션이 없어 레코드는 서로 독립이다 — 끊을 이유가
     없고, 끊으면 폭발 반경만 N 배가 된다. 지금은 **실패한 건만** 센다(반경 N → 1).
  🔑 대기 예산(`send-timeout-ms`)은 **전체가 하나를 공유**한다. 건별로 주면 브로커 장애 때
     count × 시한이 되어 요청 스레드를 수 분간 잡는다.
  ⚠️ **실패분은 자동 복구되지 않는다.** admit은 실패해도 200이라 Tenant가 재시도할 이유가 없다 —
     "복구는 REPLAY"는 같은 requestId로 **마침** 다시 불렀을 때만 성립하는 가능성이지 경로가 아니다.
  실패는 로그(실패 건수 + 시도 건수)와 `result=error` 로 남는다.
...
count 상한: **300**. @Max(300) 한 줄로 강제한다 (전용 검증 클래스 만들지 않는다).
  🔧 **2026-09-22 에 100 → 300 으로 올렸다**(커밋 0082a01). 이 절의 옛 서술은 100이었다.
  🔧 **옛 근거("Redis 단일 스레드라 수십~100ms")는 실측의 1000배 과대평가였다**(AWS 5차):
     ZPOPMIN N=20 이 14.7μs, 토큰당 0.7μs. **막는 것은 Redis 가 아니다.**
  🔴 **진짜 제약은 Tenant 의 좌석 회전율이다.** admitToken TTL 60초는 발급 순간부터 흐르므로
     받은 N 장을 μ(초당 앉히는 수)로 앉히면 꼬리는 N/μ 초 뒤에 앉는다 — 그 값이 60을 넘는
     만큼이 만료분이다. 안전값은 Little(L = λW)로 정한다:
         count ≤ μ × 60 × 안전계수
     실측(2026-09-23 로컬, N=300 · μ=2~4/s): 발급의 **29%**가 TTL 을 넘겨 404 373건.
     즉 **300 은 허용 상한이고 안전값은 Tenant 마다 다르다.** 산정법은
     doc/TENANT_INTEGRATION.md §5, 필드 설명은 doc/API.md.
  상한을 올리는 건 하위호환이지만 **내리는 건 파괴적 변경**이다. 근거·상향 절차는 §80 ⑦.
