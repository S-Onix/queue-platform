...
public interface QueueJpaRepository extends JpaRepository<QueueEntity, Long> {
     ...
     * 원인: 상속 CRUD 에는 클래스 레벨 {@code readOnly} 가 걸려 있고, 선언한 파생 쿼리는 트랜잭션이 안 열려 master 다 — 기준은 <b>"readOnly 트랜잭션이 열렸는가" 하나</b>다(§4-3).
   ...
   List<QueueEntity> findAllBy();
     ...
     * 🪤 <b>근거를 "파생 쿼리라서"로 적지 마라</b> — 호출자가 package-private 이라 {@code @Transactional} 이 <b>아예 안 걸린다</b>.
