 ...
 * 🔴 <b>Sentinel 분기를 코드에서 지웠다</b> — 프로파일로 나누면 <b>"Cluster 에서만 터지는" 결함이
 *    숨을 통로</b>가 생긴다(인프라는 학습 자산으로 보존, §75 D28).
...
public class RedisConfig {
     ...
     * 문제: 🔴 <b>"재시도하면 회복된다"가 아니다</b> — Lua 가 이미 성공했으면 재시도가 EXISTS 로
     *       떨어져 발행이 스킵되고 <b>DB 에 없는 좀비</b>가 된다. 반경도 <b>청크 하나(최대 500건)</b> 다.
     * 해결: 5초 — 드러나는 실패(5xx)를 SIGKILL 로 인한 검출 불가 유실보다 택했다(창은 60초에도 있었고
     *       5초는 <b>빈도만 바꾼다</b>). 대가는 failover 5~10초를 못 넘는 것이다.
