...
-- KEYS[3]: token key (예: queue:{q_bts}:tokens) — identifier -> tokenId 매핑 Hash
...
-- ARGV[3..]: identifier1, tokenId1, identifier2, tokenId2, ...   (아이템당 2개)
--   score는 Lua가 INCR로 발급. tokenId는 Java에서 발급한 후보로,
--   OK일 때만 채택되고 EXISTS/FULL이면 버려진다.

-- Returns: {{identifier, tokenId, status, rank, total}, ...}
...
for i = 1, requestCount do
	local identifier = ARGV[2 * i + 1]   -- ARGV[3]부터 identifier (score 없으므로 오프셋 변경)
	local tokenId = ARGV[2 * i + 2]
	...
	-- score를 INCR로 발급 (큐별 전역 순번, 단조증가)
		...
		-- EXISTS (이미 존재 — INCR한 seq는 버려짐, 순서엔 영향 없음)
			...
			local existingToken = redis.call('HGET', tokenKey, identifier)
			if not existingToken then existingToken = '' end
			...
			table.insert(enqueueResults, {identifier, existingToken, "EXISTS", existingRank, currentSize, existingSeq})
		else
		-- OK
			redis.call('HSET', tokenKey, identifier, tokenId)
			local newRank = redis.call('ZRANK', queueKey, identifier)
			currentSize = currentSize + 1
			table.insert(enqueueResults, {identifier, tokenId, "OK", newRank, currentSize, seq})
		end
	end
end
