...
public enum ErrorCode {
    ...
    /**
     * 이유: 지금 상태에서 갈 수 없는 전이를 요청했다(예: ACTIVE 큐에 바로 DELETE).
     * 문제: 5xx 로 답하면 Tenant 가 영원히 재시도한다.
     * 원인: 호출자가 순서를 틀린 것이라 재시도로 해결되지 않는다.
     * 해결: 409 로 답한다. 명세가 곧 SDK 라 이 구분이 계약의 일부다(§35).
     * 🪤 {@code complete} 에는 쓰지 않는다 — 거긴 원인을 안 가리고 404 하나다(FRS §6.6).
     */
