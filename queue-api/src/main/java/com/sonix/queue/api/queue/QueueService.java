...
public class QueueService {
     ...
     * 이미 발급된 토큰은 청구 대상이라 지우면 과금 근거가 사라진다. 반면 Redis는 지운다.
     * "큐를 지우면 줄도 사라진다"가 계약이고, 안 지우면 그 메모리를 영원히 잡는다.
     *
     * <p>🔴 <b>정리가 실패하면 삭제도 실패한다 — 큐는 ACTIVE로 남는다.</b>
     * 예전 주석은 "정리가 실패하면 상태만 DELETED로 남고 회수 배치가 걷는다"고 적었는데
     * <b>거짓이다</b>(2026-09-18 3인 검토). {@code purgeDeleted}가 이 트랜잭션 안이고 예외를
     * 삼키지 않아, 던지면 위 {@code save}가 <b>함께 롤백</b>된다. 그래서 Redis 단절 중에는
     * {@code DELETE}가 매번 500이고 "지울 수 없는 큐"가 된다 — 재시도로만 풀린다.
     * 🪤 그래도 정리를 저장보다 <b>뒤</b>에 두는 순서는 유지한다. 먼저 지우면 저장이 실패했을 때
     *    살아 있는 큐의 대기자만 날아간다 — 그쪽이 더 나쁘다.
     */
    @Transactional
