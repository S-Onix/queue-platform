...
- **DB 시간의 86%를 쓰던 쿼리를 찾아** 인덱스 하나로 CPU 90.5% → 34.3%.
...
> 📏 verify 폴백 0.75 ms는 인덱스를 넣은 뒤의 값이다. 넣기 전 AWS에서는 **193 ms**였다(위 ③).
...
**원인** — 쿼리별 누적 시간을 보니 **하나가 DB 전체 시간의 86%**를 쓰고 있었다.
입장권을 확인하는 조회였고, 한 번에 **319만 행**을 훑으며 **회당 193ms**가 걸렸다.
...
안 골랐다**(`ANALYZE TABLE` 뒤에도). 쓰라고 명시(`FORCE INDEX`)하고서야 57 → 24ms가 됐다.
