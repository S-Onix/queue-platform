...
- **200만 명을 전 구간 통과시켰다**(AWS 13차 · 최신 코드) — 유입 2,000,001명 전원에게 입장권이 나갔고,
  원장 적재는 부하 중 최대 33.5만 건 밀렸다가 10분 안에 0으로 돌아왔다. 로컬에서는 50만 명을
...
| | 결과 |
|---|---|
| **200만 명 투입** (13차 · 2026-09-23) | 유입 **2,000,001** = 입장권 발급 **2,000,001** · 테넌트 40곳 · 95분 · 원장 적재 지연 최대 33.5만 건 → **10분 안에 0** |
| **묻기 — 50만 명분(초당 25,448)** (11차) | **초당 25,432번** 처리 · 응답 **5.62ms** · 실패 0 · 서버 CPU **최대 47.5%** |
| 줄 세우기 — 초당 3,000명 (13차) | p95 **28.7ms** · p99 **33.1ms** |
| 입장 확인 · 완료 · 전광판 (13차) | verify p95 **9.8ms** · complete p95 **26.6ms** · status p95 **0.97ms** |
| **입장시키기 — 300명씩** (12차 → 13차) | p99 **2.14초 → 27.9ms** (아래 참조) |

🔴 **"맞다"와 "제때 맞다"는 다르다.** 적재는 비동기라 부하가 몰리면 원장이 뒤처진다. 200만 판에서도
부하 30분 무렵 **33.5만 건이 밀렸고**, 그 사이에는 "지금 몇 명이 들어왔나"에 원장이 답할 수 없다 —
**비동기 적재의 대가가 여기서 숫자로 보인다.** 다만 10분 안에 0으로 돌아왔고 판 끝까지 0이었다.
> 📏 더 큰 규모(500만)는 적재·인덱스·발행 경로를 고치기 **전** 코드로만 돌려봤다. 최신 코드로 다시 재기 전까지 싣지 않는다.

> 📏 **p95** = 100번 중 95번은 이 시간 안에 답했다는 뜻이다. 평균은 느린 소수를 가려버려서 쓰지 않는다.
> 13차 값은 부하 중 15초 구간별 백분위의 **중앙값**이다(서버 히스토그램, 원본 CSV 보관).
...
- 🪤 **500만 명은 최신 코드로 다시 재야 한다.** 이전 코드로는 돌려봤지만 그 뒤 적재·인덱스·발행 경로가 바뀌었다
