...
[![Tests](https://img.shields.io/badge/Tests-526-success)](#-테스트-전략)
...
| **입장시키기 — 300명씩** | 응답 **2.14초 → 27.5ms** (아래 참조) |
...
🔴 **인덱스를 만들어도 옵티마이저가 안 골랐다.** 기존 인덱스가 더 싸다고 판단해서,
쓰라고 명시(`FORCE INDEX`)하기 전까지는 아무 효과가 없었다. "만들었는데 안 빨라진다"로 한참 헤맸다.
...
**526건**(전체, 벤치마크 4건 skip) / **337건**(단위 레인). 가르는 기준은 모듈이 아니라 **`@Tag`**다 — 실 MySQL·Redis
...
| 항목 | 지금 | 남은 것 |
|---|---|---|
| 알람 | 규칙 23개 → Alertmanager → **Slack·webhook 도달 확인**(실패 0) | 로컬은 `nohup` 이라 재부팅하면 사라진다 |
| 지표 노출 | prod 에서 켰다. 경계는 **관리 포트 분리** — 공개 포트에서 actuator 가 **404** 로 사라진다 | 관리 포트엔 **인증이 없다.** 네트워크(SG)가 유일한 문이다 |
| 롤백 | 커밋 SHA 로 태그 → `rollback` 한 줄. 로컬에서 **5초** 실증 | **AWS 에서 아직 안 돌려 봤다** |
