...
**③ 알려진 결함 4건 (미조치)**
- 🔴 **통합 테스트 17건이 날짜 때문에 깨진다.** 과금 테스트가 대상 월을 `2026-07`로 **하드코딩**해
  뒀는데, 파티션 정리 배치가 규칙(M+2월)대로 그 달을 **실제로 지워버렸다**. 즉 시간이 지나면
  스스로 깨지는 종류다 — `ERROR 1735 Unknown partition`.
  🔑 이 결함이 오늘까지 안 보인 이유는 **단위 레인만 돌렸기 때문**이다(그쪽은 337건 전부 초록).
  "테스트가 초록이다"가 무엇의 초록인지 다시 확인해야 한다는 사례가 하나 더 늘었다
